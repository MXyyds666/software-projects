...
extern uint8_t ID_FLAG;
extern uint8_t ID_CMD;
...
static int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t *Len)
{
  ...
  /* 模式切换序列检测: AA AA AA / BB BB BB */
  if (ModeManager_DetectSequence(Buf, *Len)) {
  ...
  {
    /* 12字节: 扩展ID(4B) + 8字节数据 → 直接发CAN, 无需关心当前模式 */
    Protocol_HandleSingleCanFrame(Buf, *Len);
  }
  else if (ModeManager_GetCurrentMode() == MODE_PROTOCOL_BRIDGE)
  {
    /* BB 模式非12字节数据: 丢弃 */
  }
  else
