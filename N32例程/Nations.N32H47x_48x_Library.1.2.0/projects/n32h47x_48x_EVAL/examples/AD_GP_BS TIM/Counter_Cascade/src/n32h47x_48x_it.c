...
/**
*\*\name    TIMx_UP_IRQHandler.
*\*\fun     This function handles TIM1_UP_IRQHandler.
*\*\param   none
*\*\return  none 
**/
void TIMx_UP_IRQHandler(void)
{
    if (TIM_GetIntStatus(TIMx,TIM_INT_UPDATE) != RESET)
    {
        ...
        /* Pin GPIOA.05 toggling */
        GPIO_WriteBits(GPIOA, GPIO_PIN_5, (Bit_OperateType)(1 - GPIO_ReadOutputDataBit(GPIOA, GPIO_PIN_5)));
    }
}

void TIMy_UP_IRQHandler(void)
{
    if (TIM_GetIntStatus(TIMy, TIM_INT_UPDATE) != RESET)
    {
        TIM_ClrIntPendingBit(TIMy, TIM_INT_UPDATE);

        /* Pin GPIOA.06 toggling */
        GPIO_WriteBits(GPIOA, GPIO_PIN_6, (Bit_OperateType)(1 - GPIO_ReadOutputDataBit(GPIOA, GPIO_PIN_6)));
    }
}
